...
GLESDebugDraw::GLESDebugDraw()
    : mRatio( 1.0f ), mvOrigin( 0.0f, 0.0f )
{
    this->initShader();
}

GLESDebugDraw::GLESDebugDraw( float32 ratio )
    : mRatio( ratio ), mvOrigin( 0.0f, 0.0f )
{
    this->initShader();
}
...
void GLESDebugDraw::DrawCircle(const b2Vec2& center, float32 radius, const b2Color& color)
{
    ...
    {
        b2Vec2 v = (center - mvOrigin) + radius * b2Vec2(cosf(theta), sinf(theta));
...
void GLESDebugDraw::DrawSegment(const b2Vec2& p1, const b2Vec2& p2, const b2Color& color)
{
    ...
    GLfloat    glVertices[] = 
    {
        p1.x * mRatio - mvOrigin.x, p1.y * mRatio - mvOrigin.y,
        p2.x * mRatio - mvOrigin.x, p2.y * mRatio - mvOrigin.y
    };
    glVertexAttribPointer(kCCVertexAttrib_Position, 2, GL_FLOAT, GL_FALSE, 0, glVertices);
...
void GLESDebugDraw::SetScreenOrigin( float originX, float originY )
{
	mvOrigin.Set( originX, originY );
}
